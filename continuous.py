        ...
        for ii, arg in enumerate(args):
            if isinstance(arg, (list, tuple, np.ndarray)):
                if np.ndim(arg) == 1:
                    ...
                    raise ValueError(
                        "Arrays should have one or two dimensions.")
...
def unit_interval(arr):
    return (arr - np.nanmin(arr, axis=0)[None, :]) / (np.nanmax(arr, axis=0) -
...
def get_pmi_mvn(x, y, z):
    ...
    hxyz = 0.5 * log(
        (2 * np.pi * np.e)**(3 * d) * det(np.cov(np.c_[x, y, z].T)))
...
def get_h(x, k=1, norm='max', min_dist=0., workers=1):
    ...
    sum_log_dist = np.sum(log(
        2 * distances))  # where did the 2 come from? radius -> diameter
        ...
        for ii in range(n):
            nx[ii] = len(
                x_tree.query_ball_point(
                    x_tree.data[ii], r=epsilon[ii], p=p, workers=workers)) - 1
            ny[ii] = len(
                y_tree.query_ball_point(
                    y_tree.data[ii], r=epsilon[ii], p=p, workers=workers)) - 1

        mi = digamma(k) - np.mean(digamma(nx + 1) + digamma(ny + 1)) + digamma(
            n)  # version (1)
        # mi = digamma(k) -1./k -np.mean(digamma(nx) + digamma(ny)) + digamma(n) # version (2)

    else:
        raise NotImplementedError(
            "Estimator is one of 'naive', 'ksg'; currently: {}".format(
                estimator))
...
def get_pmi(x,
    ...
    if estimator == 'naive':
        ...
        for ii in range(n):
            nz[ii] = len(
                z_tree.query_ball_point(
                    z_tree.data[ii], r=epsilon[ii], p=p, workers=workers)) - 1
            nxz[ii] = len(
                xz_tree.query_ball_point(
                    xz_tree.data[ii], r=epsilon[ii], p=p, workers=workers)) - 1
            nyz[ii] = len(
                yz_tree.query_ball_point(
                    yz_tree.data[ii], r=epsilon[ii], p=p, workers=workers)) - 1

        pmi = digamma(k) + np.mean(
        ...
        rxz, _ = xz_tree.query(
            xz, k=k + 1, p=p,
            workers=workers)  # +1 to account for distance to itself
        ryz, _ = yz_tree.query(
            xz, k=k + 1, p=p, workers=workers
        )  # +1 to account for distance to itself; xz NOT a typo

        pmi = yz.shape[1] * np.mean(
            log(ryz[:, -1]) - log(rxz[:, -1]))  # + log(n) -log(n-1) -1.

    else:
        raise NotImplementedError(
            "Estimator one of 'naive', 'fp', 'ps'; currently: {}".format(
                estimator))
...
def get_imin(x1, x2, y, k=1, normalize=None, norm='max', workers=1):
    ...
    for jj, x in enumerate([x1, x2]):
        ...
        i_spec[jj] = digamma(k) - digamma(nx + 1) + digamma(ny + 1) + digamma(
            n)  # version (1)
