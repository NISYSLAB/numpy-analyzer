...
def create_filter(data,fs,f_low=None,f_high=None,order=2,method='iir', 
    ...
    if method.lower() == 'iir':
        if output == 'ba':
            ...
            filter = (b,a)
            w,h = get_freq_response(filter,output,analog)
        ...
        w,h = get_freq_response(filter,output,analog=False)
