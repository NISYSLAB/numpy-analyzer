...
import librosa
from segment_data import make_views
...
def build_features_dataframe(dataset_records,features_names):
    ...
    for i,record in enumerate(dataset_records):
        ...
        data = record[0]
        label = record[1]
        ch1,ch2,ch3 = separate_data_chs(data)
        clean_chs.append(np.array(ibdct(ch1)).reshape(-1,1))
        clean_chs.append(np.array(ibdct(ch2)).reshape(-1,1))
        clean_chs.append(np.array(ibdct(ch3)).reshape(-1,1))
        for j in range(len(clean_chs)):
            ch_df = get_feature_df_single_channel(clean_chs[j],120,60,2,0,0.03,0.25,features_names,label,(j+1),(i+1))
